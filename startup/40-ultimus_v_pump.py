...
from extra.errors import print_error
#from pump import Pump
from extra.A3200 import A3200
    ...
    def disconnect(self, stopPump = False):
        if self.debug:
            ...
            if self.pump.isOpen():
                if stopPump:
                    self.stopPump()
                    #time.sleep(self.dispense_delay) #wait a second for dispensing to stop 
                self.pump.close()
                if self.pump.isOpen():
                    return 0
                else:
...
def test():
    ''' 
        Run this to test the pump
    '''
    #a = A3200()    
    pump = Ultimus_V_Pump("/dev/ttyUSB0")
    pump.connect()
    time.sleep(1)
